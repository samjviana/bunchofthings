...
public class ColoredPistonTileEntity extends TileEntity implements ITickableTileEntity {
    ...
    public ColoredPistonTileEntity() {
       super(ModTileEntityType.COLORED_PISTON.get());
    }
 
    public ColoredPistonTileEntity(BlockState pistonStateIn, Direction pistonFacingIn, boolean extendingIn, boolean shouldHeadBeRenderedIn) {
       this();
    ...
    private BlockState getCollisionRelatedBlockState() {
       return !this.isExtending() && this.shouldPistonHeadBeRendered() && this.pistonState.getBlock() instanceof ColoredStickyPistonBlock ? ModBlocks.COLORED_STICKY_PISTON_HEAD.get().getDefaultState().with(ColoredStickyPistonHeadBlock.SHORT, Boolean.valueOf(this.progress > 0.25F)).with(ColoredStickyPistonHeadBlock.TYPE, this.pistonState.isIn(Blocks.STICKY_PISTON) ? PistonType.STICKY : PistonType.DEFAULT).with(ColoredStickyPistonHeadBlock.FACING, this.pistonState.get(ColoredStickyPistonBlock.FACING)) : this.pistonState;
    }
    ...
    public VoxelShape getCollisionShape(IBlockReader p_195508_1_, BlockPos p_195508_2_) {
        ...
        if (!this.extending && this.shouldHeadBeRendered) {
            voxelshape = this.pistonState.with(ColoredStickyPistonBlock.EXTENDED, Boolean.valueOf(true)).getCollisionShape(p_195508_1_, p_195508_2_);
        } else {
        ...
        if ((double)this.progress < 1.0D && direction == this.getMotionDirection()) {
            ...
            BlockState blockstate;
                if (this.shouldPistonHeadBeRendered()) {
                    blockstate = ModBlocks.COLORED_STICKY_PISTON_HEAD.get().getDefaultState().with(
                    ColoredStickyPistonHeadBlock.FACING, this.pistonFacing
                    ).with(
                    ColoredStickyPistonHeadBlock.SHORT, Boolean.valueOf(this.extending != 1.0F - this.progress < 0.25F)
                    ).with(
                    ColoredStickyPistonHeadBlock.COLOR, this.pistonState.get(ColoredStickyPistonHeadBlock.COLOR)
                    );
                } else {
