...
public class ColoredStickyPistonBlock extends PistonBlock {
    ...
    public ColoredStickyPistonBlock(Colors color, boolean sticky, Properties properties) {
        ...
        this.setDefaultState(this.stateContainer.getBaseState().with(
            ...
            EXTENDED, Boolean.valueOf(false)
        ).with(
            COLOR, color
        ));
    ...
    public boolean eventReceived(BlockState state, World worldIn, BlockPos pos, int id, int param) {
        ...
        if (id == 0) {
            ...
            if (this.isSticky) {
                ...
                if (!flag1) {
                    if (id != 1 || blockstate1.isAir() || !canPush(blockstate1, worldIn, blockpos, direction.getOpposite(), false, direction) || blockstate1.getPushReaction() != PushReaction.NORMAL && !blockstate1.isIn(Blocks.PISTON) && !blockstate1.isIn(Blocks.STICKY_PISTON) && !blockstate1.isIn(ModBlocks.COLORED_STICKY_PISTON_HEAD.get())) {
        ...
        if (pos.getY() >= 0 && pos.getY() <= worldIn.getHeight() - 1 && worldIn.getWorldBorder().contains(pos)) {
            if (blockStateIn.isAir()) {
                ...
                if (facing == Direction.DOWN && pos.getY() == 0) {
                    ...
                    if (!blockStateIn.isIn(Blocks.PISTON) && !blockStateIn.isIn(Blocks.STICKY_PISTON) && !blockStateIn.isIn(ModBlocks.COLORED_STICKY_PISTON_HEAD.get())) {
    ...
    private boolean doMove(World worldIn, BlockPos pos, Direction directionIn, boolean extending, BlockState state) {
        ...
        if (!extending && worldIn.getBlockState(blockpos).isIn(
            ModBlocks.COLORED_STICKY_PISTON_HEAD.get().getDefaultState().with(
                ColoredStickyPistonHeadBlock.COLOR,  state.get(COLOR)
            ).getBlock()
        )) {
        ...
        if (!pistonblockstructurehelper.canMove()) {
            ...
            for(int l = list.size() - 1; l >= 0; --l) {
                ...
                worldIn.setTileEntity(blockpos3, new ColoredPistonTileEntity(list1.get(l), directionIn, extending, false));
                ablockstate[j++] = blockstate5;
            }

            if (extending) {
                PistonType pistontype = this.isSticky ? PistonType.STICKY : PistonType.DEFAULT;
                BlockState blockstate4 = ModBlocks.COLORED_STICKY_PISTON_HEAD.get().getDefaultState().with(
                    PistonHeadBlock.FACING, directionIn
                ).with(
                    PistonHeadBlock.TYPE, pistontype
                ).with(
                    ColoredStickyPistonHeadBlock.COLOR,  state.get(COLOR)
                );
            ...
            if (extending) {
                worldIn.notifyNeighborsOfStateChange(blockpos, ModBlocks.COLORED_STICKY_PISTON_HEAD.get().getDefaultState().with(
                    ColoredStickyPistonHeadBlock.COLOR, state.get(COLOR)
                ).getBlock());
            }
